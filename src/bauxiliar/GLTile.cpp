...
void GLTile::draw(glm::mat4 model)
{
    glm::mat4 tmp = model * transform;
    tmp = glm::translate(tmp, glm::vec3(-hot_x,-hot_y,0));
...
{
    glm::mat4 tmp = glm::translate(transform, glm::vec3(x,y,z));
    if (scale!=1) tmp = glm::scale(tmp, glm::vec3(scale, scale, scale));
    if (angle!=0) tmp = glm::rotate(tmp, angle, glm::vec3(0,0,-1));
